...
- **로그인 시도 제한**: (아이디 + IP)별 · IP별로 15분 안에 5번 틀리면 15분 동안 로그인을 막는다(`lib/login-limit.ts`). 아이디만으로 잠그면 남이 일부러 틀려 관리자를 잠글 수 있어 IP와 묶어 센다. 시도를 먼저 기록한 뒤 판단해서 동시에 보낸 요청도 한도를 넘지 못한다. 서버리스라 메모리 대신 DB(`LoginAttempt`)에 기록한다.
