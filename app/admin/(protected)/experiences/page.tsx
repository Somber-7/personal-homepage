        ...
        <div className="p-6 rounded-xl mb-6" style={{ background: "var(--surface)", border: "1px solid var(--accent)" }}>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <div className="md:col-span-2">
              <Field label="업무 설명 (한 줄에 하나씩 · 라벨: 내용 형식이면 라벨이 굵게)" value={form.desc} onChange={(v) => setForm({ ...form, desc: v })} multiline />
            </div>
          ...
          <div key={item.id} className="p-4 rounded-xl flex items-start justify-between gap-4" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                ...
                {item.isCurrent && (
                  <span className="text-xs px-2 py-0.5 rounded-full font-mono" style={{ background: "rgba(47,107,79,0.12)", color: "var(--accent-green)" }}>현재</span>
                )}
              </div>
