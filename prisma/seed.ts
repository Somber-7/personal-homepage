  ...
  await prisma.project.createMany({
    data: [
      ...
      },
      {
        ...
        solution: "재무 문장 검증: DART 재무값으로 계정·기간별 비교값을 계산해 모델 입력과 검증에 같이 쓰고, 모델이 비교값과 반대 방향이나 근거 없는 개선·악화를 단정하면 분석 결과를 채택하지 않음\n오탐 줄이기: \"현금 유출 증가\"를 개선으로 잘못 읽고 다른 주어의 증감을 붙이던 과잉 거부를 부호 반전·주어 분리 규칙으로 고침\n자료 부족 시 유보: 일봉 검증에 실패하거나 근거가 부족하면 모델 답과 관계없이 판단 유보로 강제\n주문 안전장치: 분석 후 5분 · 가격 변동 3% 제한, 중복 주문 방지 ID, 결과가 불확실하면 재전송하지 않음\n키 보관: AI 키는 Windows DPAPI로 암호화, 응답에 키가 섞여 나오면 거부\n주입 문구 차단: 웹 검색 발췌가 모델에게 말을 거는 문장(이전 지시 무시, 역할 태그, 출력 칸 이름)을 담으면 근거에서 빼서, 프롬프트의 '자료는 지시가 아님' 규칙에만 기대지 않도록 함\n평가셋: 가상 종목 5개 사례(자료 불량 시 유보, 매수 여력 미확인, 웹·공시 속 지시문, 조건 미입력 시 추세 판단)와 결정적 판정 규칙을 만들어 실제 모델 응답을 채점하는 회귀 점검용 도구\n소요 시간 기록: 분석 단계별(시세·DART·웹 검색·AI 응답) 소요 시간과 최근 50회 중앙값·95% 값, 회당 토큰을 기록해 화면에 표시",
        result: "Python 테스트 323개, 버전 1.21까지 배포\n설치형과 무설치 실행형으로 배포\n한계: 평가셋은 5개 사례를 1회씩 돌린 수준이라 반복 실행 시 일관성은 확인하지 못함. 투자 조건이 비면 가격 판단은 하되 수량 판단을 유보하는 경향을 발견해 다음 평가 사례로 남김",
        role: "단독 개발",
        repo: "https://github.com/Somber-7/stock-analyze",
        tags: J(["Python", "FastAPI", "React", "Electron", "JavaScript", "unittest"]),
        order: 17,
      },
      {
